...
/**
 * SystemTest — Full Coverage System Tests
 *
 * Covers 100% of:
 *   • onetoone.realtime_chat   (ChatController, ChatServer, ChatMessage, ChatService, WebSocketConfig, WebConfig, DTOs)
 *   • onetoone.AiChat          (AiChatController, AiChatService, AiChatMessage, DTOs)
 *   • onetoone.Users           (UserController, UserAdminController, User, DailyCheckIn, LoginRequest/Response, Role, DTOs)
 *   • onetoone.Notes           (NoteController, NoteService, Note, DTOs)
 *   • onetoone.Appointments    (AppointmentController, AppointmentService, Appointment, DTOs)
 *   • onetoone.Tasks           (TaskController, TaskService, Task, TaskStatus, DTOs)
 *   • onetoone.Notification    (NotificationController, NotificationService, Notification, NotificationType, WebSocket)
 *   • onetoone.Counsellors     (CounsellorProfileController, CounsellorProfileService, CounsellorProfile, CounsellorStatus, DTOs)
 *
 * Run the entire class in order — do NOT run tests individually.
 */
...
public class ShreySystemTest {
    ...
    void tc13_webSocketChatServer() throws Exception {
        ...
        /*
         * We open TWO WebSocket sessions so the server can exercise the
