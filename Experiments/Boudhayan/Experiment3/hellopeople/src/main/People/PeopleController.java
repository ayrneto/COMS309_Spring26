...
public class PeopleController {
    ...
    //Displays the details of a single person
    @PathVariable("/people/{p}")
    public String display(@PathVariable Person p){
        return p.toString();
    }

    //Checks if the person can drink alcohol or not by age
    @PathVariable("/people/{p}")
    public String ofAge(@PathVariable Person p){
        if(p.getAge()>=21)
            return "The person can drink";

        return "The person cannot drink"
    }
